...
export function Header({children, clean=false}: HeaderProps){
    ...
    return (
        <div className='flex h-16 w-full justify-between place-items-center bg-deep-blue px-10 text-bright-white items-center'>
            ...
            {children}
            {isLogged() ? <Link to="/"><CardButton label="Logout" icon={<MdCancel size={25} onClick={(e)=>{logout(); navegador('/login');}}/>}/> </Link> :<Link to="/login"> <CardButton icon={<HiOutlineUserCircle size={39} className=""/>}/></Link>}
        </div>
